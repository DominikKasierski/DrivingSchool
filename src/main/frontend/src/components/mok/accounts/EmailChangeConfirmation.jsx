...
function EmailChangeConfirmation() {
    ...
    const handleSubmit = () => {
        axios.put("/resources/account/confirmEmail/" + code, {}, {}).then((res) => {
            dispatchSuccessNotification({
                message: i18n.t('email.change.confirmation.success')
            })
            history.push("/signIn");
        }).catch(err => {
        ...
        <div className="container-fluid">
            <Breadcrumb>
                ...
                <li className="breadcrumb-item active text-secondary"
                    aria-current="page">{i18n.t('edit.own.account.edit.email.address')}</li>
            </Breadcrumb>
            <Container>
                <Row>
                    <Col xs={12} sm={11} md={9} lg={8} xl={7} className={"floating py-3 mx-auto mb-3 mt-4"}>
                        <div className="py-2">
                            <h1 className="font-weight-light text-center">{i18n.t("edit.own.account.edit.email.address")}</h1>

                            <div className="col-12 text-center mt-4 mb-4">
                                <span className={"text-center"}>{i18n.t('email.change.confirmation.info')}</span>
                            </div>

                            <div className="col-12 d-flex justify-content-center mt-2">
                                <button className="btn btn-dark dim" type="button" onClick={() => handleConfirmation()}>
                                    {i18n.t('edit.own.account.edit.email.address')}
                                </button>
                            </div>
                        </div>
                    </Col>
...
export default withNamespaces()(EmailChangeConfirmation);
